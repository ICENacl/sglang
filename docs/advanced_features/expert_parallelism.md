...
If you see an error like `'LazyValue' object has no attribute 'keys'` when enabling EPLB on a MoE model, upgrade to a build that exposes `routed_experts_weights_of_layer` as a realized mapping instead of the lazy wrapper.
