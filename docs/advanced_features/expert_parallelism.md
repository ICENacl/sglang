...
SGLang also supports an async EPLB migration backend via `--enable-eplb-async`. In this mode, MoE expert weights are mirrored in shared host memory on `/dev/shm`, copied into a GPU temp buffer on a dedicated EPLB CUDA stream after each rebalance trigger, and then committed to live model parameters after a CUDA event dependency. When `--eplb-rebalance-num-iterations=N`, the async path keeps the existing rebalance trigger timing, but excludes iteration `N` itself from the statistics window so the rebalance after that iteration only consumes earlier expert-distribution data. This mode requires `--enable-eplb`, only supports CUDA EP deployments, and is incompatible with Elastic EP or online weight updates.
